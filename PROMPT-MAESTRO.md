...
**Documento v1.50 · Aplicación V GMM 0048 · 15 de agosto de 2026**
...
- **NO las cierres al perder el foco del campo** (V GMM 0048; antes sí, con ~160 ms de margen,
  pero era un resto de cuando las sugerencias eran un cuadro pequeño anclado al campo). Con la
  cuadrícula a pantalla completa, la propia cuadrícula tiene controles interactivos —favorita,
  reproducir, descargar, otra carátula—, y clicar CUALQUIERA de ellos también le quita el foco al
  campo: cerrar por eso cerraría la cuadrícula delante del clic que se estuviera dando. El cierre
  correcto es por **"clic fuera"**, comprobado contra el propio clic (`e.target`), no contra el
  foco del campo: solo cierra si el clic no cae ni dentro del campo ni dentro de la cuadrícula de
  sugerencias.

**Las sugerencias aparecen EN LA PÁGINA, como una cuadrícula a pantalla completa que oculta los
carruseles mientras está activa — nunca en un cuadro desplegable flotando sobre el contenido, ni
confinadas a una fila estrecha dentro de la barra fija** (V GMM 0046; la 0045 ya las sacó del
cuadro desplegable pero las dejó como una fila horizontal dentro de `.chrome-fijo`, con los
carruseles todavía visibles justo debajo compitiendo por sitio — el usuario pidió expresamente
que los carruseles desaparecieran mientras se busca, y que las carátulas salieran «a tamaño
completo, en buena calidad, y en toda la pantalla si fuese necesario», con una app de gestión de
biblioteca ajena como referencia visual: una cuadrícula de varias columnas y filas). Implementación:

- El contenedor de sugerencias vive en el **flujo normal de la página**, hermano de la sección de
  carruseles (§5.1.4b más abajo) — no dentro de la barra fija, ni de ningún envoltorio `sticky`:
  al ocultarse los carruseles mientras hay sugerencias, no compite por espacio con nada, así que
  no necesita quedarse pegada arriba.
- Es una **cuadrícula que envuelve**, no una fila que se desplaza en horizontal: **la misma
...
- **Mientras haya alguna sugerencia que mostrar, oculta los carruseles, el título del buscador y
  lo que hubiera en el área de resultados** (la bienvenida, o una ficha si se escribe encima de un
  resultado ya abierto) — la cuadrícula de sugerencias debe ser lo único a la vista, sin
  distracciones. **Al cerrarse, no intentes recordar "cómo estaba la pantalla antes"**: reutiliza
  la misma función que ya decide qué mostrar según el estado de la vista actual (inicio o
  resultados) — es el mismo problema que ya está resuelto en otro sitio, no hace falta un segundo
...
- **El header, y desde V GMM 0044 también la barra bajo el header, son fijos juntos**
  ...
  de 4 filas** (2 hasta la 0043; 3 entre la 0044 y la 0046, con la barra de búsqueda ora en su
  fila propia, ora compartida con el interruptor — el usuario probó ambas y terminó pidiendo la
  primera, con más ancho): arriba, compacta, **Mis listas / interruptor centrado / ⚙**; debajo,
  **la barra de búsqueda, sola en su fila, a lo ancho** (spanning las tres columnas — el usuario
  pidió expresamente más anchura: compartir fila con el interruptor la dejaba «muy cortada», así
  que Mis listas y la barra de búsqueda intercambian sitio respecto a como quedaron en la 0045);
  luego, **volver, a lo ancho** (solo en resultados); al fondo, Te la tengo / Instalar. Solo CSS
...
convierte con `ffmpeg`, en cola de una a la vez, con caché en disco (el nombre temporal de la
conversión en curso **debe terminar en `.mp4`**, p. ej. `<id>.parcial-<pid>.mp4` — FFmpeg elige
el contenedor de salida por la extensión del archivo de destino, y un temporal sin extensión
reconocible hace fallar la conversión entera, remux o transcodificar por igual); `GET
/api/medios/:id` responde 202 "preparando" mientras tanto. Sin `ffmpeg` instalado, todo sigue
igual que antes. **Abrir en otro reproductor (V GMM 0039):** `GET /api/medios/:id?tipo=original`
da un enlace temporal al archivo tal cual, sin pasar por la conversión — para quien prefiera VLC,
el reproductor del móvil o una TV, que a diferencia del `<video>` del navegador leen MKV/AC3 sin
ayuda. En la PWA es `GMM.servidor.enlaceOriginal(id)` y un botón en el reproductor que copia el
enlace (`navigator.clipboard`, o `window.prompt()` si no está disponible); vive fuera del
contenedor que el sondeo de conversión repinta, para no desaparecer a media espera. **El sondeo
de conversión se cancela con un token de sesión (V GMM 0040):** `abrirReproductorServidor`
guarda un `tokenReproductor` que se incrementa en cada apertura; el bucle de `setTimeout`
recursivo que revisa el estado cada 4 s comprueba en cada paso que su copia del token sigue
vigente antes de continuar o de programarse otra vez. Comprobar solo si la capa del modal está
oculta **no basta**: cerrar la película A y abrir la de B reutiliza la misma capa sin ocultarla,
así que el sondeo de A seguía vivo sin que nadie lo parase, acumulándose con cada película nueva
que se abriera en la misma sesión hasta saturar el servidor de peticiones sueltas.
...
**Compartir el PC con la cuenta de otra persona (V GMM 0042):** distinto del acceso remoto del
propio dueño — usa el flujo de invitación de Tailscale (*Share machine*, por correo), sin meter
a la persona invitada en la tailnet personal. Documentado en `guias/tailscale-compartir-gmm/`
(carrusel HTML autónomo + PDF, con capturas reales numeradas), fuera de `index.html` y sin
enlace desde la PWA. Dos trampas de ese flujo: la invitación se acepta **dos veces** (antes y
después de iniciar sesión con la cuenta invitada) y hay que **cerrar la sesión anterior en el
iPhone** antes de conectar el dispositivo nuevo.
...
`.exe` independientes** —`GMM-Server.exe` (con el motor entero incrustado, extraído a
`%LOCALAPPDATA%\GMM-Server\motor`) y `GMM-Instalar.exe` (instala Node.js,
...
alternativa manual, sin formar parte de la entrega.

**Crítico, verificado en la práctica (V GMM 0039–0041): ps2exe re-extrae los archivos
incrustados EN CADA ARRANQUE del `.exe`, no solo la primera vez.** `servidor.js`, `src/*.js`,
`preparar.js` y `configuracion.ejemplo.json` se sobrescriben siempre con lo que lleve
incrustado el `.exe` que se ejecuta (`PRIVADO/` y el catálogo no se tocan, viven aparte). Por
tanto: **cualquier cambio a `gmm-server/src/*.js` exige recompilar con `Compilar.ps1` y copiar
los `.exe` nuevos a `GMM-Server-para-instalar/` antes de considerarlo aplicado** — parchear a
mano dentro de `%LOCALAPPDATA%\GMM-Server\motor\src\` es temporal y se pierde en el siguiente
arranque. Ningún protocolo de cierre de versión de la PWA recompila el `.exe` por su cuenta.
...
| A62 | Nombre temporal de la conversión (V GMM 0039) | El archivo temporal que `_transcodificar` le pasa a `ffmpeg` como destino **termina en `.mp4`** (p. ej. `<id>.parcial-<pid>.mp4`, nunca `<id>.mp4.parcial-<pid>`); con `ffmpeg` real, tanto remux como transcodificar completan sin el error "use a standard extension for the filename" |
...
| A80 | Interactuar con la cuadrícula no la cierra (V GMM 0048) | Con sugerencias a la vista, pulsar el corazón (favorita) de una tarjeta la marca **sin** cerrar la cuadrícula ni navegar a la ficha; el contador de Mis listas sube igual que si se hiciera desde cualquier otra cuadrícula |
...
|---|---|---|---|
| 1.50 | V GMM 0048 | 15-08-2026 | **Arregla un bug real de la 0047, encontrado por el usuario probando la app de escritorio**: tocar reproducir (o cualquier otro control de una sugerencia) cerraba la cuadrícula de resultados enterase. Causa: `#entrada` cerraba las sugerencias 160 ms después de perder el foco (`blur`) — un cierre heredado de cuando eran un cuadro desplegable pequeño, que dejó de tener sentido en la V GMM 0046 al pasar a ser una cuadrícula a pantalla completa con controles propios (favorita, reproducir, descargar): clicar cualquiera de ellos también quita el foco al campo, así que ese `blur` seguía cerrando la cuadrícula delante del clic que se estuviera dando. Se quita el manejador de `blur` entero; el cierre por "clic fuera" del `document` (que sí distingue dentro/fuera de la cuadrícula por el propio `e.target`, no por el foco) ya cubría lo necesario. Nueva trampa en `CLAUDE.md` §9. Criterio A80 nuevo. `sw.js` 42→43. `logica.js` 190/190 (sin cambio), `interfaz.js` 132→135 (marcar/quitar favorita desde una sugerencia no cierra la cuadrícula, y el contador se actualiza igual), `pwa.js` 20/20 (`gmm-app-v43`). Respaldo de la 0047 reconstruido desde su commit antes de tocar nada. |
...
| 1.42 | V GMM 0040 | 08-08-2026 | **Arreglo real, encontrado en vivo mirando la consola del usuario: un sondeo de conversión suelto machacaba el servidor.** El botón «Abrir en otro reproductor» de la 0039 seguía fallando de forma intermitente. Primera pista (descartada): el permiso nuevo de Chrome "Local Network Access" — el usuario lo activó, el fallo siguió. Pista real: la pestaña Network mostró 1045 peticiones en 5,3 minutos. Causa: `abrirReproductorServidor` paraba su sondeo solo si `#capaReproductor` estaba oculta, pero cerrar la película A y abrir la B **reutiliza la misma capa sin ocultarla**, así que el sondeo de A seguía preguntando por A cada 4 s indefinidamente; con varias películas probadas seguidas durante el propio diagnóstico, se acumularon varios sondeos sueltos a la vez. Arreglo: `tokenReproductor`, un contador que se incrementa en cada apertura; el bucle comprueba en cada paso que su copia (`miToken`) sigue vigente antes de continuar. `sw.js` 34→35. Criterio A64 (nuevo). Sin comprobaciones nuevas en `pruebas/`: la función no es exportada ni pura, mismo motivo documentado en la 0039 para esa vista. `logica.js` 190/190, `interfaz.js` 127/127, `pwa.js` 20/20. De paso: un apagón del PC a mitad de sesión no perdió el arreglo (los cambios de archivo sobreviven sin comitear), y una prueba de PWA en rojo por un icono que faltaba (`iconos/icono-512.png`, cambio suelto y anterior del propio usuario) se resolvió preguntando antes de tocar nada — el usuario ya lo había repuesto. |
| 1.41 | V GMM 0039 | 08-08-2026 | **Arreglo real: el 100% de las conversiones de FFmpeg fallaban.** Reportado por el usuario («algunos archivos, sobre todo MKV, no se reproducen») ya con FFmpeg instalado y las pruebas locales en verde — diagnosticado contra el catálogo y el servidor reales, no contra simulacros: la carpeta de caché de transcodificación estaba completamente vacía pese a 466 MKV en el catálogo. Causa: `_transcodificar` escribía a un nombre temporal `<id>.mp4.parcial-<pid>`, y FFmpeg elige el contenedor de salida por la extensión del destino — `.parcial-<pid>` no es reconocible, fallaba con "use a standard extension…" el 100% de las veces, remux y transcodificar por igual. No se detectó en la 0038 porque el usuario aún no tenía FFmpeg instalado en ese momento, y el ejecutor falso de las pruebas no reproduce ese sniffing real. Arreglo de una línea: el temporal ahora conserva `.mp4` al final. Verificado contra el servidor real del usuario (no solo el simulacro de las pruebas): copiado a la extracción real en `%LOCALAPPDATA%`, probado a mano y con una conversión real disparada por el propio usuario. Se recompilaron los dos `.exe` de distribución. **Además, nueva función:** botón «Abrir en otro reproductor» en «Te la tengo» — `GET /api/medios/:id?tipo=original` (nuevo) da el archivo tal cual sin pasar por FFmpeg, para VLC/móvil/TV, que sí leen MKV/AC3 sin ayuda; `GMM.servidor.enlaceOriginal` + botón que copia el enlace (`navigator.clipboard` o `prompt()` de reserva), fuera del contenedor que el sondeo de conversión repinta. `sw.js` 33→34. Criterios A62 y A63 (nuevos). `logica.js` 190/190 (2 nuevas; se detectó y corrigió una condición de carrera real entre dos bloques de prueba que compartían `global.fetch` — iban con `Promise.all`, tenían que ir encadenadas), `interfaz.js` 127/127, `pwa.js` 20/20, GMM Server 35/35 (1 nueva). El botón nuevo se verificó también con un script de Playwright aparte, fuera de `pruebas/`. |
